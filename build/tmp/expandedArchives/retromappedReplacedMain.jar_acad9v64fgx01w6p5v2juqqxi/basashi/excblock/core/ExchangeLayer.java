	...
	private void initConfig(){
		...
		for ( int i = 0; i < blocks.length; i++){
			...
			if (blocks[i].contains(":")){
				...
				// ブロックを取得
				Block blk = Block.func_149684_b(blocks[0]);
				int meta =0;
				try{
					// メタデータを数値化
				    meta = CommandBase.func_175764_a(divd[1], 0,16);
				}catch(Exception ex){
					// 数値にできない場合、デフォルトのブロックを使用する
					ModLog.log().error(divd[1] + "is no integer. get defaultstate.");
					meta = 0;
				}
				targetBlock[i] = blk.func_176203_a(meta);
			}else{
				targetBlock[i] = Block.func_149684_b(blocks[i]).func_176223_P();
			}
			ModLog.log().debug("target brock[" +i+"] = " + targetBlock[i].func_177230_c().getRegistryName());
		}
		...
		for ( int i = 0; i < blocks.length; i++){
			...
			if (blocks[i].contains(":")){
				...
				afterBlock[i] = blk.func_176203_a(meta);
			}else{
				afterBlock[i] = Block.func_149684_b(blocks[i]).func_176223_P();
			}
			ModLog.log().debug("brock[" +i+"] = " + afterBlock[i].func_177230_c().getRegistryName());
		}
		...
		if (world instanceof WorldServer)
		{
			((WorldServer)world).func_152344_a(
				new Runnable()
				{
					...
					public void run()
					{
						for ( int stcnt =0; stcnt < chunk.func_76587_i().length; stcnt++)
						{
							if ( (stcnt*16+15) < minHeight || (stcnt*16) > maxHeight){continue;}
							ExtendedBlockStorage strage = chunk.func_76587_i()[stcnt];
							// ストレージが空の場合コンティニュー
							if ( strage == null){
								strage = new ExtendedBlockStorage(stcnt*16,true);
								chunk.func_76587_i()[stcnt] = strage;
							}

							// yロケーションが範囲外の場合コンティニュー
							int location = strage.func_76662_d();
							...
							for (int x = 0; x < 16; x++){
								for (int z = 0; z < 16; z++){
									for ( int y = starty; y < 16; ++y){
										...
										for (int tcnt = 0; tcnt < targetBlock.length; tcnt++){
											if (strage.func_177485_a(x,y,z) == targetBlock[tcnt]){
												exec = true;
												break;
											}
										}
										...
										for ( int rcnt = 0; rcnt < blockRate.length; rcnt++){
											if (blockRate[rcnt] > val){
												try{
													strage.func_177484_a(x,y,z,afterBlock[rcnt]);
												}catch(Exception ex){
												}
												break;
											}
